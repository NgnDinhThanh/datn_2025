...
class ArucoDetectorPlugin : FlutterPlugin, MethodChannel.MethodCallHandler {
    ...
    private val COLOR_CORRECT = Scalar(0.0, 255.0, 0.0)
    private val COLOR_WRONG = Scalar(0.0, 0.0, 255.0)
    private val COLOR_SHOW_CORRECT =
        ...
        when (call.method) {
            "detectMarkers" -> {
                ...
                try {
                    ...
                    android.util.Log.d(
                        "ArucoDetector",
                        "Detection successful: ready=${detectionResult["ready"]}, markers=${detectionResult["markersNorm"]}"
                    )
            ...
            "createAnnotatedImage" -> {
                ...
                try {
                    val annotatedBase64 = createAnnotatedImage(
                        warpedImageBase64,
                        template,
                        studentAnswers,
                        correctAnswers
...
// Apply preprocessing (similar to backend)
    ...
    private fun scanAnswerSheet(
        ...
        for (question in questions) {
            ...
            val (selected, isMultiple) = detectMarkedBubbleAndDrawWithMultiple(
                warpedGray,
                annotated,
                bubbles,
                MIN_ANSWER_PIXELS
