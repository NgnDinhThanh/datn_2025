...
class GradingService {
  ...
  static Future<bool> checkAnswerKey(String quizId, String token) async {
    try {
      ...
      final url = Uri.parse(
        '$baseUrl/grading/check-answer-key/?quiz_id=$normalizedQuizId',
      );
      ...
      print(
        '[GradingService] Answer key check response: ${response.statusCode}',
      );
      ...
      if (response.statusCode == 200) {
        ...
        print(
          '[GradingService] Answer key check failed: ${response.statusCode} - ${response.body}',
        );
        return false;
      }
    } catch (e, stackTrace) {
  ...
  static Future<List<GradeModel>> getGradesForQuiz(
    String quizId,
    String token,
  ) async {
    try {
      ...
      final url = Uri.parse(
        '$baseUrl/grading/grades/by-quiz/?quiz_id=$normalizedQuizId',
      );
      ...
      if (response.statusCode == 200) {
        ...
        print(
          '[GradingService] Get grades failed: ${response.statusCode} - ${response.body}',
        );
  ...
  static Future<ItemAnalysisModel> getItemAnalysis(
    String quizId,
    String token,
  ) async {
    try {
  ...
  static Future<List<GradeModel>> getGradesForClass(
    String classCode,
    String token,
  ) async {
    try {
  ...
  static Future<List<GradeModel>> getGradesForStudent(
    String studentId,
    String token,
  ) async {
    try {
