...
class _StudentListScreenState extends State<StudentListScreen> {
  ...
  Future<void> _deleteSelectedStudents() async {
    ...
    try {
      ...
      setState(() {
        _error = 'Failed to delete selected students';
      });
    }
  }
  ...
  Future<void> exportStudents(String type) async {
    if (!kIsWeb) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Export functionality is only available on web'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final anchor = html.AnchorElement(href: blobUrl)
          ..setAttribute(
            'download',
            type == 'csv' ? 'students.csv' : 'students.xlsx',
          )
          ..click();
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Export ${type.toUpperCase()} completed successfully',
            ),
          ),
        );
      } else {
        ...
        filtered.sort((a, b) {
          ...
          switch (_sortField) {
            ...
            case 'last_name':
              ...
              break;
            case 'classes':
              final aClasses = a.classCodes
                  .map((cid) {
                    final cList = allClasses
                        .where((cl) => cl.id == cid)
                        .toList();
                    return cList.isNotEmpty ? cList.first.class_name : '';
                  })
                  .join(',');
              final bClasses = b.classCodes
                  .map((cid) {
                    final cList = allClasses
                        .where((cl) => cl.id == cid)
                        .toList();
                            ...
                            child: Center(
                              child: Checkbox(
                                value:
                                    pageData.isNotEmpty &&
                                    pageData.every(
                                      (c) => _selectedStudentIds.contains(
                                        c.studentId,
                                      ),
                                    ),
                                onChanged: (selected) {
                          ...
                          children: [
                            ...
                            TableCell(
                              verticalAlignment:
                                  TableCellVerticalAlignment.middle,
                              child: Center(
                                child: Text('${studentItem.studentId}'),
                              ),
                            ),
                            TableCell(
                              verticalAlignment:
                                  TableCellVerticalAlignment.middle,
                              child: Center(
                                child: Text('${studentItem.firstName}'),
                              ),
                            ),
                            TableCell(
                              verticalAlignment:
                                  TableCellVerticalAlignment.middle,
                              child: Center(
                                child: Text('${studentItem.lastName}'),
                              ),
                            ),
                            TableCell(
                              verticalAlignment:
                            ...
                            TableCell(
                              ...
                              child: Center(
                                child: IconButton(
                                  ...
                                  onPressed: () {
                                    context.go(
                                      '/students/${studentItem.studentId}',
                                    );
                                  },
                                ),
                              ),
                            ...
                            TableCell(
                              ...
                              child: Center(
                                child: IconButton(
                                  ...
                                  onPressed: () {
                                    context.go(
                                      '/students/${studentItem.studentId}/detail',
                                    );
                                  },
                                ),
                              ),
