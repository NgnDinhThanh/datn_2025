...
class _QuizDetailScreenState extends State<QuizDetailScreen> {
  ...
  Future<void> _reloadAnswerKeys() async {
    final answerKeyProvider = Provider.of<AnswerKeyProvider>(
      context,
      listen: false,
    );
  ...
  Future<void> _loadGrades() async {
    ...
    try {
      ...
      final grades = await GradingService.getGradesForQuiz(
        widget.quizId,
        token,
      );
      setState(() {
  ...
  Future<void> _loadItemAnalysis() async {
    ...
    try {
      ...
      final analysis = await GradingService.getItemAnalysis(
        widget.quizId,
        token,
      );
      setState(() {
  ...
  Map<String, int> _calculateScoreDistribution() {
    ...
    for (int i = 0; i < 20; i++) {
      ...
      final label =
          '${minPercent.toStringAsFixed(1)}%-${maxPercent.toStringAsFixed(1)}%';
  ...
  Widget build(BuildContext context) {
    ...
    if (answerSheetProvider.isLoading) {
      ...
      final answerSheetObj = answerSheetProvider.answerSheets.where(
        (a) => a.id == quiz.answersheet,
      );
      debugAnswerSheetIds = answerSheetProvider.answerSheets
          .map((a) => a.id)
          .join(', ');
      answerSheetName = answerSheetObj.isNotEmpty
          ? answerSheetObj.first.name
          : 'Unknown';
    }
                                ...
                                Padding(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (answerSheetName == 'Unknown' &&
                                          debugAnswerSheetIds.isNotEmpty)
                                        Text(
                                          'Debug ids: ' + debugAnswerSheetIds,
                                          style: const TextStyle(
                                            fontSize: 10,
                                            color: Colors.red,
                                          ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (answerKeyProvider.error != null)
                          Text(
                            'Error: ${answerKeyProvider.error}',
                            style: const TextStyle(color: Colors.red),
                          ),
                        if (!answerKeyProvider.isLoading &&
                            answerKeyProvider.error == null) ...[
                          ...
                          if (answerKeyProvider.answerKeys.isNotEmpty)
                            Column(
                              ...
                              children: [
                                Text(
                                  'Number of Versions: ${answerKeyProvider.answerKeys.first.numVersions}',
                                ),
                                        ...
                                        onTap: () {
                                          showDialog(
                                            ...
                                            builder: (context) {
                                              ...
                                              return AlertDialog(
                                                title: Text(
                                                  'Answers for Version: ${v['version_code']}',
                                                ),
                                                content: SizedBox(
                                                  width: 300,
                              ...
                              ElevatedButton.icon(
                                onPressed: () {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                        'Delete selected feature coming soon',
                                      ),
                                    ),
                                  );
                                },
                                ...
                                label: Text(
                                  'Delete (${_selectedGradeIds.length})',
                                ),
                                style: ElevatedButton.styleFrom(
                                  ...
                                  child: SingleChildScrollView(
                                    ...
                                    child: SingleChildScrollView(
                                      child: DataTable(
                                        ...
                                        rows: _getPaginatedGrades().map((
                                          grade,
                                        ) {
                                          final isSelected = _selectedGradeIds
                                              .contains(grade.id);
                                          return DataRow(
                                            selected: isSelected,
                                            cells: [
                                              ...
                                              DataCell(
                                                Text(
                                                  '${(grade.percentage ?? 0).toStringAsFixed(1)}%',
                                                ),
                                              ),
                                              DataCell(
                                                Text(
                                                  grade.versionCode ?? 'N/A',
                                                ),
                                              ),
                                              DataCell(
                                                Text(
                        ...
                        TableRow(
                          children: [
                            ...
                            Padding(
                              ...
                              child: Text(
                                _itemAnalysis?.totalPapers.toString() ?? '0',
                              ),
                            ),
                          ],
                        ),
                        TableRow(
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(vertical: 6),
                              child: Text(
                                _itemAnalysis?.numQuestions.toString() ?? '0',
                              ),
                            ),
                          ],
                        ),
                        ...
                        TableRow(
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(vertical: 6),
                              child: Text(
                                _itemAnalysis?.statistics.minScore
                                        .toStringAsFixed(1) ??
                                    '0',
                              ),
                            ),
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(vertical: 6),
                              child: Text(
                                _itemAnalysis?.statistics.maxScore
                                        .toStringAsFixed(1) ??
                                    '0',
                              ),
                            ),
                        ...
                        TableRow(
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(vertical: 6),
                              child: Text(
                                _itemAnalysis?.statistics.averageScore
                                        .toStringAsFixed(1) ??
                                    '0',
                              ),
                            ),
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(vertical: 6),
                              child: Text(
                                _itemAnalysis?.statistics.medianScore
                                        .toStringAsFixed(1) ??
                                    '0',
                              ),
                            ),
                        ...
                        TableRow(
                          children: [
                            ...
                            Padding(
                              padding: const EdgeInsets.symmetric(vertical: 6),
                              child: Text(
                                _itemAnalysis?.statistics.stdDeviation
                                        .toStringAsFixed(2) ??
                                    '0.00',
                              ),
                            ),
                          ...
                          child: DataTable(
                            ...
                            rows: _itemAnalysis!.items.map((item) {
                              ...
                              return DataRow(
                                cells: [
                                  DataCell(
                                    Text(item.questionNumber.toString()),
                                  ),
