...
class AnswerSheetFormPreviewScreen extends StatefulWidget {
  ...
  @override
  State<AnswerSheetFormPreviewScreen> createState() =>
      _AnswerSheetFormPreviewScreenState();
}

class _AnswerSheetFormPreviewScreenState
    extends State<AnswerSheetFormPreviewScreen> {
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      ...formProvider.headers
                          .where((h) => h.enabled)
                          .map((h) => Text('${h.label} (${h.width})')),
                      const SizedBox(height: 16),
                      Text(
                        'Student ID: ${formProvider.studentIdLabel} (${formProvider.studentIdDigits} digits)',
                      ),
                      Text(
                        'Class ID: ${formProvider.classIdLabel} (${formProvider.classIdDigits} digits)',
                      ),
                      Text(
                        'Exam ID: ${formProvider.examIdLabel} (${formProvider.examIdDigits} digits)',
                      ),
                      const SizedBox(height: 16),
                      Text(
                        'Questions: ${formProvider.numQuestions} x ${formProvider.numOptions} options',
                      ),
                      const SizedBox(height: 24),
                      ElevatedButton.icon(
                            ...
                            onPressed: () async {
                              final confirm = await showDialog<bool>(
                                ...
                                builder: (context) => AlertDialog(
                                  ...
                                  content: const Text(
                                    'Are you sure you want to publish this form? Once published, no further changes may be made to this custom form.',
                                  ),
                                  actions: [
                                    TextButton(
                                      onPressed: () =>
                              ...
                              if (confirm == true) {
                                try {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Answer sheet created!'),
                                    ),
                                  );
                                } catch (e) {
                                  Navigator.of(context).pop(); // Đóng loading
